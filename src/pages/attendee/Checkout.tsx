...
import {
    FaMoneyBillAlt, // For general payment
    FaShoppingCart, // For shopping cart related
    FaCashRegister, // For checkout specific
    FaRegCheckCircle, // For outline check circle
    FaExclamationCircle, // For outline error icon
    FaCalendarAlt, // For event/calendar
    FaCalendarDay, // For specific day calendar
    FaMapMarkerAlt, // For location
    FaMobileAlt, // For phone/mobile
    FaQrcode, // For QR code
    FaDownload, // For download
    FaCreditCard, // For credit card
} from 'react-icons/fa';
...
export const Checkout = () => {
    ...
    const handleDownloadTicket = () => {
        if (orderDetails && paymentStatus === 'success') {
            ...
            console.log('Simulated ticket download. Check your downloads!');
            setMessage('Simulated ticket download. Check your downloads!'); // Display message in UI
            setPaymentStatus('info'); // Use info for a neutral message
        }
    };
    ...
    return (
        <div className="container mx-auto p-4 md:p-8 min-h-screen">
            <h1 className="text-3xl font-bold mb-6 flex items-center gap-2">
                <FaCashRegister className="text-4xl" /> Secure Checkout
            </h1>
            ...
            {message && !checkoutModalRef.current?.open && paymentStatus !== 'success' && (
                <div role="alert" className={`alert ${paymentStatus === 'success' ? 'alert-success' : paymentStatus === 'info' ? 'alert-info' : 'alert-error'} mb-6`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 card bg-base-100 shadow-xl p-6">
                        <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
                            <FaCalendarAlt className="w-6 h-6" /> Order Summary
                        </h2>
                        <div className="space-y-4">
                            <div>
                                <p className="text-xl font-semibold mb-1">{orderDetails.eventName}</p>
                                <p className="text-sm text-base-content/70 flex items-center gap-1">
                                    <FaCalendarDay className="w-4 h-4" /> {orderDetails.eventDate}
                                </p>
                                <p className="text-sm text-base-content/70 flex items-center gap-1">
                                    <FaMapMarkerAlt className="w-4 h-4" /> {orderDetails.eventLocation}
                                </p>
                            </div>
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-4 card bg-base-100 shadow-xl p-6">
                            <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
                                <FaMoneyBillAlt className="w-6 h-6" /> Payment Method
                            </h2>
                            ...
                            >
                                ...
                                {paymentStatus === 'success' ? (
                                    <> <FaRegCheckCircle className="w-5 h-5" /> Payment Confirmed </>
                                ) : (
                                    <> <FaMoneyBillAlt className="w-5 h-5" /> Confirm and Pay ${orderDetails.totalAmount.toLocaleString('en-KE')} </>
                                )}
                            </button>
                            {paymentStatus === 'error' && (
                ...
                <div className="modal-box w-11/12 max-w-2xl">
                    ...
                    <div className="p-4 bg-base-200 rounded-box min-h-[150px] flex flex-col justify-center items-center text-center">
                        {currentStep === 1 && selectedPaymentMethod === 'mpesa' && (
                            <>
                                <FaMobileAlt className="w-16 h-16 text-primary mb-4 animate-bounce-slow" />
                                ...
                                <button
                                    ...
                                    onClick={() => { /* Implement retry logic, maybe resend STK */ console.log('Simulating STK Push retry...'); setMessage('Simulating STK Push retry...'); }}
                                    disabled={loading}
                                >
                                    Retry STK Push
                                </button>
                        ...
                        {currentStep === 1 && selectedPaymentMethod === 'card' && (
                            <>
                                <FaCreditCard className="w-16 h-16 text-primary mb-4" />
                        ...
                        {currentStep === 3 && paymentStatus === 'success' && (
                            <>
                                <FaRegCheckCircle className="w-16 h-16 text-success mb-4" />
                                ...
                                {orderDetails && (
                                    <div className="bg-white p-4 rounded-lg shadow-md mb-4">
                                        {/* Using FaQrcode directly, as the original QrCode2Icon was a component */}
                                        <FaQrcode className="w-32 h-32 mx-auto text-base-content" />
                                ...
                                >
                                    <FaDownload className="w-5 h-5 mr-2" /> Download Ticket
                                </button>
                                <button
                        ...
                        {currentStep === 3 && paymentStatus === 'error' && (
                            <>
                                <FaExclamationCircle className="w-16 h-16 text-error mb-4" />
