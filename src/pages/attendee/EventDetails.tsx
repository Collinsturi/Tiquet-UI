...
// --- React Icons Imports ---
import {
    FaCalendarAlt,      // For EventIcon
    FaMapMarkerAlt,     // For LocationOnIcon
    FaCalendarDay,      // For CalendarTodayIcon
    FaClock,            // For AccessTimeIcon
    FaInfoCircle,       // For InfoIcon
    FaTicketAlt,        // For ConfirmationNumberIcon
    FaUserTie,          // For PersonOutlineIcon (speaker)
    FaDollarSign,       // For AttachMoneyIcon (pricing)
    FaShoppingCart,     // For ShoppingCartIcon (buy tickets)
    FaShareAlt,         // For ShareIcon
    FaCalendarPlus,     // For AddAlertIcon (add to calendar)
    FaRegCheckCircle,   // For CheckCircleOutlineIcon
} from 'react-icons/fa';
        ...
        <div className="container mx-auto p-4 md:p-8">
                    ...
                    <div className="p-6 lg:p-8 w-full lg:w-1/2">
                        ...
                        <div className="flex flex-wrap gap-2">
                            <button className="btn btn-outline btn-sm">
                                <FaCalendarPlus className="w-4 h-4" /> Add to Calendar
                            </button>
                            <button className="btn btn-outline btn-sm">
                                <FaShareAlt className="w-4 h-4" /> Share Event
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 card bg-base-100 shadow-xl p-6">
                    <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
                        <FaInfoCircle className="w-6 h-6" /> Event Description
                    </h2>
                ...
                <div className="lg:col-span-1">
                    <div className="sticky top-4 card bg-base-100 shadow-xl p-6">
                        <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
                            <FaTicketAlt className="w-6 h-6" /> Get Tickets
                        </h2>
                            ...
                            {event.ticketTypes.map(type => (
                                <div key={type.id} className="flex items-center justify-between border-b border-base-200 pb-2">
                                    ...
                                    <span className="font-bold text-lg text-primary">
                                        <FaDollarSign className="inline w-4 h-4 mr-1" />{type.price.toLocaleString('en-KE')}
                                    </span>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="flex justify-between items-center text-xl font-bold mb-4">
                            <span>Total:</span>
                            <span><FaDollarSign className="inline w-5 h-5 mr-1" />{calculateTotalPrice().toLocaleString('en-KE')}</span>
                        </div>
                        ...
                        >
                            ...
                            <FaShoppingCart className="w-5 h-5" /> Buy Tickets
                        </button>
                    </div>
                </div>
            </div>
            ...
            {event.speakers && event.speakers.length > 0 && (
                <div className="card bg-base-100 shadow-xl p-6 mt-8">
                    <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
                        <FaUserTie className="w-6 h-6" /> Speakers
                    </h2>
            ...
            {event.mapImageUrl && (
                <div className="card bg-base-100 shadow-xl p-6 mt-8">
                    <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
                        <FaMapMarkerAlt className="w-6 h-6" /> Location
                    </h2>
            ...
            {event.faqs && event.faqs.length > 0 && (
                <div className="card bg-base-100 shadow-xl p-6 mt-8">
                    <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
                        <FaInfoCircle className="w-6 h-6" /> FAQs
                    </h2>
            ...
            <dialog id="buy_tickets_modal" className="modal" ref={buyTicketsModalRef}>
                <div className="modal-box">
                    <h3 className="font-bold text-lg flex items-center gap-2">
                        <FaShoppingCart className="w-6 h-6" /> Confirm Your Order
                    </h3>
                    ...
                    <div className="overflow-x-auto mb-4">
                        <table className="table w-full">
                                ...
                                .map(id => {
                                    ...
                                    return (
                                        <tr key={id}>
                                            <td>{type.name}</td>
                                            <td>{qty}</td>
                                            <td className="text-right"><FaDollarSign className="inline w-3 h-3 mr-1" />{type.price.toLocaleString('en-KE')}</td>
                                            <td className="text-right"><FaDollarSign className="inline w-3 h-3 mr-1" />{subtotal.toLocaleString('en-KE')}</td>
                                        </tr>
                                    );
                                })}
                            </tbody>
                            ...
                            <tr>
                                ...
                                <th className="text-right text-lg text-primary"><FaDollarSign className="inline w-4 h-4 mr-1" />{calculateTotalPrice().toLocaleString('en-KE')}</th>
                            </tr>
                            </tfoot>
                        </table>
                    </div>
                    <div className="modal-action">
                        ...
                        >
                            {buyTicketLoading && <span className="loading loading-spinner loading-sm mr-2"></span>}
                            <FaRegCheckCircle className="w-5 h-5" /> Proceed to Checkout
                        </button>
                    </div>
                </div>
            </dialog>
